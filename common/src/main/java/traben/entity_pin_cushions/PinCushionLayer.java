...
import traben.entity_pin_cushions.compat.TwoDProjectilesCompat;
...
public abstract class PinCushionLayer<T extends LivingEntityRenderState, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    private void renderStuckItem(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int packedLight, float x, float y, float z) {
        ...
        // Compat hook: lets ArrowLayer swap in TwoDProjectiles' flat 2D arrow item
        // instead of the vanilla 3D ArrowModel when that mod is installed and its
        // own "render arrows as 2D" option is enabled. No-op for every other layer.
        if (renderTwoDCompatStuckItem(poseStack, submitNodeCollector, packedLight)) {
            return;
        }
    ...
    /**
     * Overridden by {@link ArrowLayer} to render a flat 2D arrow item instead of
     * the vanilla 3D model when TwoDProjectiles compat is active. Returns true if
     * it handled the rendering (caller must not also draw the 3D model).
     */
    protected boolean renderTwoDCompatStuckItem(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int packedLight) {
        return false;
    }
    ...
    public static class ArrowLayer<T extends LivingEntityRenderState, M extends EntityModel<T>> extends PinCushionLayer<T, M> {
        ...
        // Reused across stuck-arrow instances/frames rather than rebuilt per arrow;
        // refreshed every submit() call via updateForNonLiving() below.
        private final ItemStackRenderState twoDArrowItemStack = new ItemStackRenderState();
        ...
        @Override
        protected boolean renderTwoDCompatStuckItem(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int packedLight) {
            if (!TwoDProjectilesCompat.isTwoDArrowActive()) {
                return false;
            }

            ItemStack stack = Items.ARROW.getDefaultInstance();
            Minecraft.getInstance().getItemModelResolver().updateForNonLiving(
                    this.twoDArrowItemStack, stack, ItemDisplayContext.GROUND, Minecraft.getInstance().player);

            float scale = TwoDProjectilesCompat.getArrowScale();
            poseStack.scale(scale, scale, scale);

            float angle = TwoDProjectilesCompat.getArrowAngle(stack);
            poseStack.mulPose(Axis.ZP.rotationDegrees(angle));

            float offset = TwoDProjectilesCompat.getArrowOffset();
            ...
            this.twoDArrowItemStack.submit(poseStack, submitNodeCollector, packedLight, OverlayTexture.NO_OVERLAY, 0);
            return true;
        }
    }
